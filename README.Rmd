...
However, Observable in the OJS + Shiny combo has the following division of labor:
...
- Plan your layout on paper (sidebars, main panels, tabs), and implement using Quarto
- Build your Observable controls first using `viewof`
- Debug the Reactive next (Observable controls will be in `input` object)
- Connect the reactive to OJS using `ojs_define()`
- Test the OJS plotting (Can also do this early with a non-reactive dataset)